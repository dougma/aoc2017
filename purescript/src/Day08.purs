...
import Prelude
import Data.Int
import Data.Array(drop, foldl, take, head)
import Data.Maybe
...
getr :: Registry -> String -> Int
getr registry key = fromMaybe 0 (lookup key registry)

setr :: Registry -> String -> Int -> Registry
setr registry key val = insert key val registry

test :: String -> Int -> Int -> Boolean
test "<" = (<)
test ">" = (>)
test "<=" = (<=)
test ">=" = (>=)
...
checkcond :: Registry -> Array String -> Boolean
checkcond r [reg, op, im] = 
    case fromString im of
        Nothing -> false
        Just val -> test op (getr r reg) val
checkcond r _ = false

update :: Registry -> Array String -> Registry
update r [reg, op, im] = 
    case fromString im, op of
        Just val, "inc" -> setr r reg ((getr r reg) + val)
        Just val, "dec" -> setr r reg ((getr r reg) - val)
        _,_ -> r
update r _ = r

parseline :: Registry -> String -> Registry
parseline r s = 
    let a = split (Pattern " ") s
    in case checkcond r (drop 4 a) of
        true -> update r (take 3 a) 
        false -> r

-- part 1

regs = foldl parseline empty lines

max :: Int -> Int -> Int
max x y | x > y = x
max x y = y

largest :: Registry -> Int -> Int 
largest r m = foldl max m $ values r

answer = largest regs 0

-- part 2

parseline2 (Tuple r m) s =
    let r2 = parseline r s
    in Tuple r2 (largest r2 m)

largest2 = foldl parseline2 (Tuple empty 0) lines
answer2 = snd largest2
