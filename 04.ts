export const raw = `nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
const valid = phrases.reduce((acc, phrase) => acc + (isValid2(phrase) ? 1 : 0), 0)
console.log(valid)
...
function isValid2(words: string[]) {
    const uniqueWords = new Set(words.map(canonicalise))
    return uniqueWords.size === words.length
}

function canonicalise(word: string) {
