...
const re = /(\w+) \((\d+)\)(?: -> (.*))?/
function parseProg(s: string) {
    const match = s.match(re)
    if (match) {
        const children = match[3] ? match[3].split(',').map(s => s.trim()) : []
        return { 
            name: match[1], 
...
const progs = lines.map(parseProg)

// part1
const progSet = new Set(progs.map(p => p.name))
for (const prog of progs) {
    prog.children.forEach(p => progSet.delete(p))
}
const root = progSet.keys().next().value
console.log(root)

// part2
const kvs = progs.map(p => [p.name, p] as [string, { weight:number, children:string[] }])
...
function weigh(p: string): number {
    const prog = get(p)
    const w = wmap.get(prog)
    if (w !== undefined) {
        return w
    }
    const ww = prog.children.reduce((acc, c) => acc + weigh(c), prog.weight)
    wmap.set(prog, ww)
    return ww
}
...
function check(p: string, shouldBe?: number) {
    console.log(p)
    const prog = get(p)
    const kids = prog.children.length
    if (kids === 0) {
        if (shouldBe === undefined) {
            return
        }
        console.log(`leaf ${p} should weigh ${shouldBe}`)
        throw "done"
    }
    const weights = prog.children.map(weigh)
    const count = (w: number) => weights.reduce((acc, val) => w === val ? acc + 1 : acc, 0)
    if (kids > 2) {
        const odd = weights.findIndex(w => count(w) === 1)
        if (odd !== -1) {
            // problem in a subtower
            const expected = weights[(odd + 1) % kids]
            check(prog.children[odd], expected)
        }
    }

    prog.children.forEach(c => check(c))
    // they were ok (it's us)

    if (shouldBe === undefined) {
        throw "non-leaf wat"
    }

    const childrenWeight = weigh(p) - prog.weight
    const result = shouldBe - childrenWeight
    console.log(`non leaf ${p} should weigh ${result}`)
    throw "done"
}